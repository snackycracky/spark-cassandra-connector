...
case class CassandraTableScan(
  ...
  private def inputRdd = {
    ...
    var rdd = context.sparkContext.cassandraTable[CassandraSQLRow](
      relation.keyspaceName, relation.tableName, ReadConf.fromSparkConf(context.conf, relation.cluster))(
      CassandraConnector(context.conf, relation.cluster), ClassTag(CassandraSQLRow.getClass),
